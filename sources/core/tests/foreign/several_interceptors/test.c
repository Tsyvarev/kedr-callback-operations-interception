/*
 * Test functionality of several foreign interceptors.
 */
...
// Another prototype object and foreign interceptor for it
struct test_prototype_object_another
{
    int some_another_fields[10];
    const struct test_operations* foreign_ops;
};

static void* get_prototype_object_another(void* data)
{
    return data;
}
...
KEDR_COI_TEST_DEFINE_FOREIGN_INTERMEDIATE_FUNC(op1_foreign_repl_another,
    get_prototype_object_another, OPERATION_OFFSET(op1), foreign_interceptor_another);
...
int test_init(void)
{
    ...
    foreign_interceptor_another = kedr_coi_foreign_interceptor_create(
        ...
        offsetof(struct test_prototype_object_another, foreign_ops),
...
int test_run(void)
{
    ...
    struct test_prototype_object_another prototype_object_another =
        {.foreign_ops = &test_operations_orig_another};
    ...
    result = kedr_coi_foreign_interceptor_watch(foreign_interceptor_another,
        &prototype_object_another);
    if(result < 0)
    {
        pr_err("Another foreign interceptor failed to watch for an object.");
        goto err_foreign_watch_another;
    }
    ...
    op1_call_counter_another = 0;
    op1_pre1_call_counter = 0;
    
    object.ops->op1(&object, &prototype_object_another);
    
    if(op1_call_counter_another == 0)
    {
        pr_err("Original operation 1 wasn't called when use another prototype.");
        result = -EINVAL;
        goto err_test;
    }
    ...
    if(op1_pre1_call_counter == 0)
    {
        pr_err("Pre handler for operation 1 wasn't called when use another prototype.");
        result = -EINVAL;
        goto err_test;
    }

    // Check another operation
    op2_call_counter_another = 0;
    op2_post1_call_counter = 0;
    ...
    if(op2_post1_call_counter == 0)
    {
        pr_err("Post handler for operation 2 wasn't called when use another prototype.");
        result = -EINVAL;
        goto err_test;
    }
    
    if(op2_call_counter_another == 0)
    {
        pr_err("Original operation 2 wasn't called when use another prototype.");
        result = -EINVAL;
        goto err_test;
    }
    ...
    if(result < 0)
    {
        pr_err("Error occured when forget normal object created from another prototype.");
        goto err_forget;
    }
    if(result == 1)
    {
    ...
    kedr_coi_foreign_interceptor_forget(foreign_interceptor_another, &prototype_object_another);
