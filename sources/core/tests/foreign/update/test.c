/*
 * Test functionality of foreign interceptor when update replacements.
 */
...
int test_run(void)
{
    ...
    if(result < 0)
    {
        pr_err("Foreign interceptor failed to update watching for an object.");
        goto err_test;
    }

    // As if operations for the prototype object was reseted.
    prototype_object.foreign_ops = &test_operations_orig;

    result = kedr_coi_foreign_interceptor_watch(foreign_interceptor,
        &prototype_object);
    if(result < 0)
    {
        pr_err("Foreign interceptor failed to update watching "
            "for an object after its operations was reseted.");
        goto err_test;
    }
