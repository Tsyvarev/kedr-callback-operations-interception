/*
 * Test functionality of creation interceptor when several watchings
 * has same tie.
 */
...
{
    int result;
    const struct test_operations* object_operations = &test_operations_orig;
    void* id = (void*)0x123;
    void* tie = (void*)0x4567;
    
    const struct test_operations* object_operations1 = &test_operations_orig;
    void* id1 = (void*)0x124;
    ...
    result = kedr_coi_factory_interceptor_watch_generic(creation_interceptor,
        id1, tie, (const void**)(&object_operations1));
    if(result < 0)
    {
        pr_err("Creation interceptor failed to watch operations with same tie.");
        goto err_creation_watch1;
    }

    if(result > 0)
    {
        pr_err("Watch for creation interceptor return %d(update indicator) "
            "with same tie as previous watch but different id.", result);
        result = -EINVAL;
        goto err_creation_watch1;
    }
    ...
    kedr_coi_factory_interceptor_forget_generic(creation_interceptor, id1,
        (const void**)&object_operations1);
    kedr_coi_factory_interceptor_forget_generic(creation_interceptor, id,
