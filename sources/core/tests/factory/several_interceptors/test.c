/*
 * Test functionality of several factory interceptors.
 */
...
// Another prototype object and factory interceptor for it
struct test_factory_another
{
    int some_another_fields[10];
    const struct test_operations* factory_ops;
};

static void* get_factory_another(void* data)
{
    return data;
}
...
int test_run(void)
{
    ...
    struct test_factory_another factory_another =
        {.factory_ops = &test_operations_orig_another};
    ...
    if(result < 0)
    {
        pr_err("Another factory interceptor failed to watch for an object.");
        goto err_factory_watch_another;
    }
    ...
    // operation 1 was called.
    object.ops = factory_another.factory_ops;

    op1_call_counter_another = 0;
    op1_pre1_call_counter = 0;
    
    object.ops->op1(&object, &factory_another);
