/*
 * Test functionality of factory interceptor when update replacements.
 */
...
int test_run(void)
{
    ...
    if(result < 0)
    {
        pr_err("Factory interceptor failed to update watching for an object.");
        goto err_test;
    }

    // As if operations for the prototype object was reseted.
    factory.factory_ops = &test_operations_orig;

    result = kedr_coi_factory_interceptor_watch(factory_interceptor,
        &factory);
    if(result < 0)
    {
        pr_err("Factory interceptor failed to update watching "
