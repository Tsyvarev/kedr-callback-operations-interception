...
static struct test_factory factory = {.factory_ops = &test_operations_orig};
...
int test_init(void)
{
    ...
    if(factory_interceptor == NULL)
    {
        ...
        goto err_factory_interceptor;
    }

    kedr_coi_factory_interceptor_trace_unforgotten_object(factory_interceptor, cb);

    return 0;

err_factory_interceptor:
...
int test_run(void)
{
    ...
    result = kedr_coi_factory_interceptor_watch(factory_interceptor, &factory);
    if(result < 0)
    {
