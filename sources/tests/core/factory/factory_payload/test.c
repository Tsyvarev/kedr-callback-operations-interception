/*
 * Test that factory payload works.
 */
...
// Factory type and factory interceptor
struct test_factory
{
...
// Factory payload
int op1_pre1_factory_call_counter;
KEDR_COI_TEST_DEFINE_PRE_HANDLER_FUNC(op1_pre1_factory, op1_pre1_factory_call_counter)

int op1_post1_factory_call_counter;
KEDR_COI_TEST_DEFINE_POST_HANDLER_FUNC(op1_post1_factory, op1_post1_factory_call_counter)


static struct kedr_coi_pre_handler pre_handlers_factory[] =
{
    PRE_HANDLER(op1, op1_pre1_factory),
    kedr_coi_pre_handler_end
};

static struct kedr_coi_post_handler post_handlers_factory[] =
{
    POST_HANDLER(op1, op1_post1_factory),
    kedr_coi_post_handler_end
};

static struct kedr_coi_payload payload_factory =
{
    .pre_handlers = pre_handlers_factory,
    .post_handlers = post_handlers_factory
};
...
int test_run(void)
{
    ...
    result = kedr_coi_factory_payload_register(factory_interceptor, &payload_factory);
    
    if(result)
    {
        pr_err("Failed to register payload for factory.");
        goto err_payload_factory;
    }
    ...
    if(op1_pre1_factory_call_counter == 0)
    {
        pr_err("Factory pre handler for operation 1 wasn't called.");
        result = -EINVAL;
        goto err_test;
    }

    if(op1_post1_factory_call_counter == 0)
    {
        pr_err("Factory post handler for operation 1 wasn't called.");
        result = -EINVAL;
        goto err_test;
    }
    ...
    kedr_coi_factory_payload_unregister(factory_interceptor, &payload_factory);
