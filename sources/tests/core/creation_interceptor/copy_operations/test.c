/*
 * Test functionality of creation interceptor when copy operations.
 */
...
int test_run(void)
{
    ...
    // As if normal object was created with watched operations,
    // but with another tie, and its operation 1 was called.
    ...
    if(op1_pre1_call_counter > 0)
    {
        pr_err("Pre handler for operation 1 was called, "
            "but object is created from tie which is not watched.");
        result = -EINVAL;
        goto err_test;
    }
