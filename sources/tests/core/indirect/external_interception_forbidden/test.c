...
static struct kedr_coi_intermediate intermediate_operations[] =
{
    ...
    INTERMEDIATE_INTERNAL_ONLY(op2, op2_repl),
    INTERMEDIATE_FINAL
};
...
KEDR_COI_TEST_DEFINE_POST_HANDLER_FUNC(op2_post2, op2_post2_call_counter)
...
static struct kedr_coi_post_handler post_handlers[] =
{
    ...
    POST_HANDLER_EXTERNAL(op2, op2_post2),
...
int test_run(void)
{
    ...
    if(result == 0)
    {
        pr_err("External handler has been registered for callback, "
