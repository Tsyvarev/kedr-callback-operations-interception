/*
 *  Test that external interception handler is called even in case of
 * NULL-pointer to operation.
 */
...
{
    int result;
    struct test_object object = {.op2 = &op2_orig};
    ...
    {
        pr_err("External pre handler for operation 1(NULL) wasn't called.");
        result = EINVAL;
        goto err_test;
    }
